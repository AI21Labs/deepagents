...
class SubagentStartEvent(TypedDict):
    """A subagent began running inside a `eval` call."""
...
class SubagentCompleteEvent(TypedDict):
    """A subagent finished successfully inside a `eval` call."""
...
class SubagentErrorEvent(TypedDict):
    """A subagent raised before returning inside a `eval` call."""
...
async def call_subagent_task_tool(
    ...
    *,
    ...
    """Call the Deep Agents task tool and return a JSON-friendly value.
